...
from key_management.domain import jwt, public_key, key_management

def it_generates_a_jwt(new_kek,
                       rotate_sig_key_pair,
                       id_token_claims):

    signed_jwt = jwt.generate_signed_jwt(id_token_claims)

    assert len(signed_jwt.split(".")) == 3


def it_validates_the_jwt(new_kek,
                         rotate_sig_key_pair,
                         id_token_claims):
    signed_jwt = jwt.generate_signed_jwt(id_token_claims)

    id_token = jwt.decode_and_validate(signed_jwt)

    assert id_token.is_right()
    assert id_token.value['sub'] == '1@selene'

def it_fails_when_the_token_is_invalid(new_kek,
                                       rotate_sig_key_pair):
    ...
    id_token = jwt.decode_and_validate(t)
